        ...
        @else
            <div class="bg-white rounded-lg shadow border border-gray-100 p-6 sm:p-8">
                ...
                <form method="POST" action="{{ route('demandes.store') }}" enctype="multipart/form-data" class="mt-6 space-y-5"
                    ...
                    <div>
                        <label for="type_document_id" class="block text-sm font-medium text-gray-700">Type de document {!! $requiredMark !!}</label>
                        <select x-ref="typeDocument" x-on:change="updateChamps($event.target); clearFieldState('type_document_id')" x-on:blur="validateField('type_document_id', $event.target)" class="mt-1 block w-full rounded-md border-gray-300 focus:border-senegal-green focus:ring-senegal-green" x-bind:class="fieldClass('type_document_id')" name="type_document_id" id="type_document_id" required>
                        ...
                        <x-input-error :messages="$errors->get('type_document_id')" class="mt-2 rounded bg-red-50 px-3 py-2" />
                    </div>
                    ...
                    <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
                        <div>
                            <label for="statut" class="block text-sm font-medium text-gray-700">Statut {!! $requiredMark !!}</label>
                            <select x-model="statut" class="mt-1 block w-full rounded-md border-gray-300 focus:border-senegal-green focus:ring-senegal-green" x-bind:class="fieldClass('statut')" x-on:change="clearFieldState('statut')" x-on:blur="validateField('statut', $event.target)" name="statut" id="statut" required>
                                <option value="">-- Choisir --</option>
